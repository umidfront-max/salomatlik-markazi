...
const { data: detail } = await useAsyncData(`doctor_${route.params.slug}`, () =>
...
<template>
  <section class="section pb-100 container">
    <DoctorDetailCard type="doctor" :data="detail" class="doctor-detail" />
    ...
    <div class="section-education">
      <div class="section-education__item" v-for="item in education.result ?? []">
        <i class="section-education__circle"></i>
        <span class="section-education__year">{{ item.url_link }}</span>
        <p class="section-education__title">{{ item.title }}</p>
      </div>
    </div>
  </section>
  <SectionNews :is-page="true" :list="news?.result ?? []" class="pb-80" />
  <section class="section container pt-80">
    ...
    <ClientOnly>
      <Swiper v-bind="swiperOptions">
        <SwiperSlide class="section-slide" v-for="item in detail.media_list ?? []">
          <Image :src="item.link ?? ''" />
        </SwiperSlide>
      </Swiper>
      <template #fallback>
