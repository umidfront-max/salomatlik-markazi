...
<template>
  <section class="section section--xs-swiper-auto">
    <div class="container">
      <div v-if="hasHeader" class="section-header" data-aos="zoom-in-up" data-aos-duration="300" data-aos-offset="300">
        <h2 class="section-title">
          {{ !isDiseases ? $t("news.title") : $t("diseases") }}
        </h2>
        <NuxtLink class="section-link" :to="localePath('news')" v-if="!isDiseases">
          {{ $t("viewAll") }}
          <Icon class="section-link__icon" name="angle-right" />
        </NuxtLink>
      </div>
      <div class="cards">
        <NuxtLink class="card" v-for="(item, index) in list" :class="{ active: index === 1 }"
          :to="localePath(`/${isDiseases ? 'static' : 'news'}/${item.slug}`)">
          ...
          <div class="card-info">
            ...
            <div class="card-info__bottom">
              <span class="card-info__date" v-if="item.date">
                <Icon name="calendar" />
                {{ item.date }}
              </span>
